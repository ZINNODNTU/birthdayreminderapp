...
param(
 ...
 [switch]$RequiresReinstall, [string]$MigrationMessage='', [string]$Channel='stable', [switch]$Mandatory
)
...
$metadata=[ordered]@{schemaVersion=1;version=$Version;buildNumber=$BuildNumber;channel=$Channel;mandatory=[bool]$Mandatory;minimumSupportedVersion=if($MinimumSupportedVersion){$MinimumSupportedVersion}else{$null};requiresReinstall=[bool]$RequiresReinstall;migrationMessage=if($MigrationMessage){$MigrationMessage}else{$null};apk=[ordered]@{name=$apk.Name;sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $apk.FullName).Hash.ToUpperInvariant();size=$apk.Length}}
